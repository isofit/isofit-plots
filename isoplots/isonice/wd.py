...
class FileFinder:
    ...
    def ifin(self, name, all=False):
        ...
        if not all:
            if len(found) > 1:
                self.log.warning(f"{len(found)} files were found containing the provided name {name!r}, try being more specific. Returning just the first instance")
    ...
    def match(self, regex, all=False):
        ...
        if not all:
            if len(found) > 1:
                self.log.warning(f"{len(found)} files were found containing the provided regex {regex!r}, try being more specific. Returning just the first instance")
    ...
    def __init__(self, *args, **kwargs):
        ...
        for d in dirs:
            for name, cls in self.classes.items():
                if name in d:
                    self.log.debug(f"Initializing {d} with class {cls.__name__}")
